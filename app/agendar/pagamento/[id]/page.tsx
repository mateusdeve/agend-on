...
export default function PaymentPage({ params }: { params: { id: string } }) {
  ...
  const handlePaymentComplete = async (paymentId: string) => {
    ...
    try {
      ...
      const { error } = await supabase.from("appointments").update({ status: "scheduled" }).eq("id", appointment.id)
      ...
      <div className="max-w-2xl mx-auto">
        {paymentComplete ? (
          <div className="text-center py-12 space-y-4">
            ...
            <p className="text-muted-foreground">Seu agendamento foi confirmado com sucesso.</p>
            <div className="pt-4">
              <Button asChild>
                <Link href="/paciente/agendamentos">Ver meus agendamentos</Link>
              </Button>
