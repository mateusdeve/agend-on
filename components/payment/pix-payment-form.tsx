...
export function PixPaymentForm({ amount, onPaymentComplete, isLoading, pixData }: PixPaymentFormProps) {
  ...
  const [qrCodeImage, setQrCodeImage] = useState(
    pixData?.qrCodeBase64 || "/placeholder.svg?height=200&width=200&text=QR+Code+PIX",
  )
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <div className="mb-4">
          <div className="bg-muted inline-flex p-4 rounded-lg">
            {qrCodeImage.startsWith("data:image") ? (
              <img src={qrCodeImage || "/placeholder.svg"} alt="QR Code PIX" className="h-48 w-48" />
            ) : (
              <img
        ...
        <div className="text-sm text-muted-foreground mb-2">Escaneie o QR code acima ou copie o código PIX abaixo</div>
        ...
        <div className="flex items-center justify-center mb-4">
          ...
          <Button variant="ghost" size="icon" onClick={handleCopyCode} className="ml-2" disabled={isLoading}>
        ...
        <div className="text-center space-y-2 mb-6">
          ...
          <p className="text-sm text-muted-foreground">
            O QR Code expira em: <span className="font-medium">{formatTime(countdown)}</span>
          </p>
        </div>
      </div>
