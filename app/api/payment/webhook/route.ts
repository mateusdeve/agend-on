  ...
  try {
    ...
    if (action === "payment.updated" || action === "payment.created") {
      ...
      if (paymentError || !payment) {
        ...
        return NextResponse.json({ success: false, message: "Pagamento não encontrado" })
      }
      ...
      if (paymentInfo.status === "approved" && payment.status !== "approved") {
        await supabase.from("appointments").update({ status: "scheduled" }).eq("id", payment.appointment_id)
      }
