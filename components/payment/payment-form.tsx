...
export function PaymentForm({ appointmentData, onPaymentComplete, appointmentId }: PaymentFormProps) {
  ...
  const handlePayWithPix = async () => {
    ...
    try {
      ...
      const pixInfo = data.payment.payment_data.point_of_interaction?.transaction_data
      ...
      toast({
        ...
        description: error.message || "Ocorreu um erro ao gerar o pagamento PIX. Tente novamente.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  const handlePixPaymentComplete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/payment/check?appointmentId=${appointmentId}`, {
        method: "GET",
      })
      ...
      if (data.status === "approved") {
        ...
        toast({
          title: "Pagamento pendente",
          description: "Seu pagamento ainda não foi confirmado. Aguarde alguns instantes e tente novamente.",
        })
      }
    } catch (error: any) {
      toast({
        title: "Erro ao verificar pagamento",
        description: error.message || "Ocorreu um erro ao verificar seu pagamento. Tente novamente mais tarde.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>Escolha a forma de pagamento para confirmar seu agendamento</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="mb-6 space-y-2">
          <h3 className="text-sm font-medium text-muted-foreground">Resumo do agendamento</h3>
          <div className="bg-muted/50 p-4 rounded-md space-y-2">
            <div className="flex justify-between">
              <span>Especialidade:</span>
              <span className="font-medium">{appointmentData.specialtyName}</span>
            </div>
            <div className="flex justify-between">
              <span>Data:</span>
              <span className="font-medium">{appointmentData.date.toLocaleDateString("pt-BR")}</span>
            </div>
            <div className="flex justify-between">
              <span>Horário:</span>
            ...
            <div className="flex justify-between border-t pt-2 mt-2">
              ...
              <span className="font-bold text-primary">
                {appointmentData.specialtyPrice ? formatCurrency(appointmentData.specialtyPrice) : "Preço não definido"}
              </span>
            </div>
          </div>
        </div>

        <Tabs defaultValue="credit_card" onValueChange={(value) => setPaymentMethod(value as "credit_card" | "pix")}>
